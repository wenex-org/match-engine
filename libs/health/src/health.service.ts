...
export class HealthService {
  ...
  public check() {
    ...
    ['disk', 'memory', 'mongo', 'redis', 'kafka', 'grpc'].forEach((check: Check) => {
      ...
      if (exists) {
        if (typeof exists === 'string') {
          ...
          if (['grpc'].includes(check)) {
            indicators.push(
              this[`${check}Status`](exists.key, exists.service as never, exists.options),
            );
          } else {
  ...
  protected grpcStatus(
    key = 'grpc',
    service?: string,
    options?: CheckGRPCServiceOptions,
  ) {
    return () => this.grpcIndicator.checkService(key, service, options);
  }
